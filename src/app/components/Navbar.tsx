...
function PlaygroundLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <Link
      href={href}
      className="bg-gray-200 text-slate-600 hover:text-purple-800 hover:bg-gray-300 px-4 py-2 rounded-lg shadow-sm hover:shadow-md hover:outline outline-purple-800 transition-all duration-200 text-sm font-medium flex items-center space-x-2"
    >
      <span>{children}</span>
...
function MobilePlaygroundLink({
  ...
  return (
    <Link
      href={href}
      onClick={onClick}
      className="bg-indigo-600 text-white hover:bg-purple-700 block px-3 py-2 rounded-md text-base font-medium transition-colors duration-200 flex items-center justify-center space-x-2"
    >
      <span>{children}</span>
      <FaArrowRight className="w-3 h-3" />
    </Link>
